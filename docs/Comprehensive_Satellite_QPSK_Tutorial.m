...
%     恢复出经LDPC编码后的原始数据。
%
% 9.  LDPC解码 (本教程中省略): 在实际系统中，解扰后的数据会经过LDPC解码器进行信道纠错。
%     (详见5.6节关于此步骤的说明)。
%
% 10. 数据输出: 将恢复的比特流转换为字节，并以十六进制格式显示。
...
% 问题背景:
%
% 经过载波同步后，虽然星座图已经稳定，但由于QPSK星座点具有π/2的旋转对称性，
% PLL锁相环可能锁定在四个等效的相位状态中的任意一个:
%
% 1. 正确相位 (0°偏移): (1+j, -1+j, -1-j, 1-j)
%
% 2. 90°相位偏移: (-1+j, -1-j, 1-j, 1+j)
%
% 3. 180°相位偏移: (-1-j, 1-j, 1+j, -1+j)
%
% 4. 270°相位偏移: (1-j, 1+j, -1+j, -1-j)
%
% 理论解决方案:
%
% 利用已知的帧同步字序列(1ACFFC1D)作为"相位参考"，通过相关性检测来确定正确的相位:
%
% 1. 相位穷举法: 对接收信号依次进行0°、90°、180°、270°的相位校正
%
% 2. 同步字匹配: 将每种相位校正后的信号与已知同步字进行匹配
%
% 3. 最优选择: 选择匹配度最高的相位作为正确的相位校正量
%
% 数学表达式:
%
% 设接收到的符号为r[n]，四种相位校正为:
...
% φ₀ = 0°: s₀[n] = r[n]
%
% φ₁ = 90°: s₁[n] = r[n] × e^(-jπ/2) = r[n] × (-j)
%
% φ₂ = 180°: s₂[n] = r[n] × e^(-jπ) = r[n] × (-1)
%
% φ₃ = 270°: s₃[n] = r[n] × e^(-j3π/2) = r[n] × j
%
% 相关性度量: ρₖ = |||∑ᵢ sₖ[i] × sync*[i]|², k ∈ {0,1,2,3}
%
% 最优相位: φ* = argmax(ρₖ)
%

%% 5.5.2 帧同步理论基础
% 帧同步的目标是在连续的符号流中准确定位帧边界
%
% CCSDS AOS帧结构回顾:
%
% - 帧长度: 1024字节 = 8192比特 = 4096符号 (QPSK)
%
% - 同步字: 1ACFFC1D (32比特) = 16符号
%
% - AOS帧头: 6字节 = 48比特 = 24符号
%
% - 数据负载: 886字节
...
% 同步字设计原理:
%
% 同步字1ACFFC1D经过特殊设计，具有优良的自相关特性:
%
% 1. 尖锐的自相关峰: 在零延迟时相关值最大
%
% 2. 低旁瓣: 在其他延迟位置相关值很小
%
% 3. 抗噪声能力: 即使在低信噪比下也能可靠检测
%
% 自相关函数: R(τ) = ∑ᵢ sync[i] × sync*[i-τ]
%
% 理想情况下: R(0) >> R(τ), τ ≠ 0
%
% 滑动窗检测算法:
...
% 1. 用同步字在接收序列上进行滑动相关
%
% 2. 当相关峰值超过门限时，标记为可能的帧起始位置
%
% 3. 结合相位校正，同时解决相位模糊和帧同步问题
%

%% 5.5.3 解扰理论原理
% 解扰是加扰操作的逆过程，用于恢复原始数据
%
% 加扰的目的:
%
% 在卫星通信中，加扰的主要目的包括:
%
% 1. 频谱均匀化: 避免数据中长串0或1造成的频谱集中
%
% 2. DC平衡: 消除直流分量，改善链路性能
%
% 3. 能量分散: 使信号能量在频域内均匀分布
%
% 4. 同步性能: 提高接收端时钟恢复和载波恢复的性能
%
% CCSDS标准加扰器:
%
% 根据CCSDS标准，采用生成多项式 G(x) = 1 + X¹⁴ + X¹⁵ 的线性反馈移位寄存器(LFSR)
%
% LFSR结构: 15级移位寄存器，反馈连接在第14级和第15级
%
% I/Q路独立初始状态:
%
% - I路初相: 111111111111111 (全1序列)
%
% - Q路初相: 000000011111111 (前8位0，后7位1)
%
% 这种设计确保I路和Q路产生不同的伪随机序列，避免相互干扰
%
% 解扰验证机制:
%
% 本实现采用帧尾验证位检查解扰正确性:
...
% - 验证条件: 这两位必须同时为00
%
% - 物理意义: 这是AOS帧格式中的特定标志位，正确解扰后应为固定值
%
% 详细技术原理 - 为什么是8159-8160位:
%
% 根据CCSDS AOS帧结构和LDPC编码规范:
%
% 1. **LDPC系统码特性**:
%
%    - 本系统采用LDPC系统码编码方式
%
%    - LDPC系统码的特点是信息位保持不变，只在数据末尾添加校验位
%
%    - 信息部分: 前6048位 (6字节AOS帧头 + 750字节数据负载)
%
%    - 校验部分: 后1024位 (128字节LDPC校验码)
%
% 2. **帧末尾校验位设计**:
%
%    - 根据CCSDS-131.0-B-5标准，AOS帧的末尾两个比特是校验位
%
%    - 这两个校验位的位置是8159和8160 (从1开始计数)
%
%    - 设置目的: 作为解扰验证的可靠判据
%
%    - 重要说明: 这两位是专门的校验位，不是信息载荷的一部分
%
% 3. **解扰验证原理**:
%
%    - 发送端: 将校验位8159-8160位设置为"00"
%
%    - 传输过程: 经过加扰、调制、传输、解调、解扰等环节
%
%    - 接收端: 如果解扰正确，校验位8159-8160位应恢复为"00"
%
%    - 验证逻辑: 检查校验位是否为"00"来判断整个解扰过程的正确性
%
% 4. **为什么选择校验位作为验证**:
%
%    - **抗突发错误**: 末尾校验位受信道突发错误影响相对较小
%
%    - **编码完整性**: 校验位能反映整帧数据的完整性和正确性
%
%    - **实现简便**: 无需复杂的CRC计算，简单的校验位检查即可
%
%    - **标准规范**: 符合CCSDS国际标准，具有权威性和可靠性
%
%    - **验证有效性**: 校验位的正确性直接反映了整个解扰过程的正确性
%
% 5. **验证失败的可能原因**:
%
%    - IQ路交换: 最常见原因，通过自动IQ交换检测解决
%
%    - 相位模糊未正确恢复: 同步字匹配错误导致
%
%    - 解扰多项式或初相错误: 算法实现问题
%
%    - 帧同步位置偏移: 同步字检测偏差导致帧边界错误
%
%    - 信道误码率过高: 超出纠错编码能力范围
%
% 这种验证机制的优势:
%
% ✓ 标准化: 基于国际CCSDS标准，具有权威性
%
% ✓ 高效性: 只需检查2个比特，计算量极小
%
% ✓ 可靠性: 错误概率为1/4，提供充分的验证可信度
%
% ✓ 实时性: 无需等待整帧处理完成，可即时验证
%
% 自适应IQ路交换检测:
%
...
%% 5.6 关于LDPC解码的说明
% 读者可能会注意到，在解扰之后，我们的接收机链路直接进入了帧头验证和数据输出环节，
% 而没有实现AOS帧结构中定义的LDPC（低密度奇偶校验码）解码步骤。
% 这是一个基于实际工程和教学考虑的有意简化，原因如下：
%
% 1.  **教学目标的聚焦**：本教程的核心目标是让学习者掌握数字接收机中的关键同步技术
%     （定时、载波、帧同步）和基础解调解扰流程。LDPC解码本身是一个复杂的信道编码领域课题，
%     其算法（如置信度传播算法）的实现细节会分散学习重点。
%
% 2.  **工具链与性能限制**：学生在实践中发现，其使用的MATLAB R2021a版本未提供直接适配
%     AOS帧标准的LDPC解码函数。自行编写的解码器若未经过高度的稀疏矩阵运算优化，
%     处理大规模校验矩阵时会消耗极大的计算资源，难以在个人计算机上高效运行。
%
% 3.  **高信噪比的实际情况**：本次实验采用的真实卫星数据信噪比（SNR）非常高，
%     从清晰的载波同步后星座图就可以看出。这意味着在传输过程中引入的误码极少。
%     因此，即使跳过LDPC纠错步骤，解调出的数据也具有很高的准确性，足以通过帧头校验和恢复数据。
%
% 4.  **LDPC系统码的特性**：该卫星信号采用了LDPC的“系统码”形式。
%     系统码的一个重要优点是，编码后的码字直接包含了原始的、未经改变的信息位。
%     128字节的LDPC校验位仅仅是附加在892字节（6字节帧头+886字节数据）的原始信息之后。
%     因此，在不需要纠错的情况下，我们可以直接“截断”掉末尾的128字节校验位，
%     从而直接获得原始传输的数据负载。
%
% **重要提示**：
% 必须强调，在真实的、信道条件多变的卫星通信系统中，LDPC解码是不可或缺的关键环节。
%
% 它是保障在低信噪比下依然能够可靠通信的最后一道防线。
%
% 在未来的学习或工程实践中，深入研究和实现高效的LDPC解码器将是必然要求。
...
%
% 2. 解扰验证: 检查8159-8160位是否为00
%
% 3. IQ路检测: 自动检测和纠正IQ路交换
%
% 4. 数据完整性: 统计解扰成功的帧数量
%
% 成功判断标准:
...
% 定义AOS帧头结构表格
aos_header_table = table(...
    {'1-2'; '3-10'; '11-16'; '17-40'; '41'; '42'; '43-44'; '45-48'}, ...
    {'Version'; 'Spacecraft ID'; 'Virtual Channel ID'; 'Frame Count'; 'Replay Flag'; 'VC Usage Flag'; 'Spare'; 'Frame Count Cycle'}, ...
    {2; 8; 6; 24; 1; 1; 2; 4}, ...
    {'传输帧版本号'; '航天器标识符'; '虚拟信道标识符'; '虚拟信道帧计数器'; '回放标识'; '虚拟信道帧计数用法标识'; '备用位'; '帧计数周期'}, ...
    'VariableNames', {'比特位置', '字段名称', '比特长度', '描述'});

% 显示表格
disp('=== AOS帧头结构表（CCSDS标准）===');
disp(aos_header_table);
fprintf('\n');
