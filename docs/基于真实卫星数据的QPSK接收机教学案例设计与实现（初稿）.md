...
本教学案例的核心目标是培养学生深入理解QPSK解调及同步理论、掌握数字接收机整体架构、具备处理真实数据和解决实际工程问题的综合能力。为此，我们基于CCSDS（空间数据系统咨询委员会）标准，设定了明确的技术路线：采用QPSK调制，符号率为75 MBaud/s，脉冲成形为滚降系数α=0.33的RRC滤波，同步策略为Gardner定时同步与PLL载波同步，并遵循AOS（先进轨道系统）帧结构。

为激发学生的自主性和创造性，案例采用了开放式模块化设计，支持学生根据自身技术背景选择不同实现路径。
...
#### 3.2.1 RRC匹配滤波器
根升余弦（RRC）滤波器是数字通信中最大化信噪比和减小码间串扰的关键[7]。其设计重点在于理解滚降系数α对带宽效率和抗定时误差能力的影响，以及收发两端RRC滤波器级联后构成升余弦（RC）滤波器的匹配滤波原理。在本案例中，滤波器滚降系数α=0.33，覆盖8个符号长度，并在重采样后实现每符号3个采样点。
...
本案例作为“通信系统仿真与建模”课程的可选提高项目，引导学生在4至6周内完成从设计、开发到答辩的全过程。项目的开放性体现在技术路径的自主选择上，三组学生基于自身背景和兴趣分别选择了纯MATLAB、混合架构和向量化优化的实现方案，他们的实践成果充分验证了该教学模式的有效性。
...
**汪曈熙的混合架构路径**侧重于系统级建模与工程实践。他利用Simulink的可视化能力清晰地搭建了信号处理链路，并成功集成了标准的CCSDS LDPC解码器模块[8]。在实践中，他发现了AGC与Gardner环路间的相互干扰问题，并通过参数调整予以解决，这充分体现了其系统联调与工程问题解决能力。
